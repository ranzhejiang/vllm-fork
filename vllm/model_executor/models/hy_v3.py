...
class HYV3MoEBlock(nn.Module):
    ...
    def __init__(
        ...
        router_scaling_factor = float(_cfg(config, "router_scaling_factor", 1.0))
        ...
        if self.num_shared_experts > 0:
            self.shared_mlp = HYV3MLP(
                ...
                intermediate_size=self.expert_hidden_dim * self.num_shared_experts,
...
class HYV3Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.make_empty_intermediate_tensors = make_empty_intermediate_tensors_factory(
            ["hidden_states", "residual"], int(config.hidden_size)
        )
        ...
        self.num_logical_experts = int(config.num_experts) if example_moe else None
    ...
    def forward(
        ...
        for i in range(self.start_layer, self.end_layer):
            hidden_states, residual = self.layers[i](
                positions, hidden_states, residual, idx=i
            )
...
class HYV3ForCausalLM(nn.Module, SupportsPP, SupportsLoRA):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        ─────────────────────────────────────────────────────────────────
        self_attn.{q,k,v}_proj.weight            → self_attn.qkv_proj   (stacked)
        mlp.{gate,up}_proj.weight (dense)        → mlp.gate_up_proj      (stacked)
        mlp.shared_mlp.{gate,up}_proj.weight     → mlp.shared_mlp.gate_up_proj (stacked)
        mlp.experts.<eid>.{gate,up,down}_proj    → mlp.experts           (FusedMoE)
        mlp.router.gate.weight                   → mlp.router.gate.weight
        mlp.expert_bias                          → mlp.expert_bias        (Parameter, direct)
        ...
        for name, loaded_weight in weights:
            if "rotary_emb.inv_freq" in name:
                continue
            if bool(_cfg(self.config, "tie_word_embeddings", False)) and "lm_head.weight" in name:
                continue
